...
void getCofactor(vector<vector<double>>& mat, vector<vector<double>>& temp, int p, int q, int n) {
    ...
    for (int row = 0; row < n; row++) {
        for (int col = 0; col < n; col++) {
            ...
            if (row != p && col != q) {
                temp[i][j++] = mat[row][col]; // Sao chép phần tử vào temp
...
double determinant(vector<vector<double>>& mat, int n) {
    if (n == 1) return mat[0][0]; // Trường hợp cơ bản cho ma trận 1x1

    vector<vector<double>> temp(n - 1, vector<double>(n - 1)); // Ma trận tạm thời để lưu các ma trận con
    ...
    for (int f = 0; f < n; f++) {
        getCofactor(mat, temp, 0, f, n); // Lấy phần tử con của mat[0][f]
        det += sign * mat[0][f] * determinant(temp, n - 1); // Thêm phần tử con vào định thức
        sign = -sign; // Đổi dấu
    }
...
void adjoint(vector<vector<double>>& mat, vector<vector<double>>& adj, int n) {
    ...
    vector<vector<double>> temp(n - 1, vector<double>(n - 1)); // Ma trận tạm thời để lưu các phần tử con
    int sign;

    // Vòng lặp để tìm ma trận phụ hợp bằng cách tính các phần tử con và dấu của chúng
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            getCofactor(mat, temp, i, j, n); // Lấy phần tử con của mat[i][j]
            sign = ((i + j) % 2 == 0) ? 1 : -1; // Dấu cho phần tử con
            adj[j][i] = (sign) * (determinant(temp, n - 1)); // Lưu ma trận phụ hợp
        }
    }
}
...
bool inverse(vector<vector<double>>& mat, vector<vector<double>>& inv, int n) {
    double det = determinant(mat, n); // Tính định thức của ma trận
    ...
    adjoint(mat, adj, n); // Tìm ma trận phụ hợp
...
int main() {
    ...
    vector<vector<double>> mat(n, vector<double>(n)); // Ma trận để lưu dữ liệu nhập vào
    ...
    if (inverse(mat, inv, n)) { // Nếu tồn tại ma trận nghịch đảo, in ra nó
